...
export const useButterflyStore = create<ButterflySimStoreState>((set, get) => ({
  deltaDeg: 0.001,
  editMode: "synced",
  ...
  _updateSide: (side, state, energy, derived) => {
    ...
    } as Partial<ButterflySimStoreState>);
  },

  _setWorkerReady: (side, ready) => {
    const sideKey = side === "A" ? "sideA" : "sideB";
    const current = get()[sideKey];
    set({ [sideKey]: { ...current, workerReady: ready } } as Partial<ButterflySimStoreState>);
  },
